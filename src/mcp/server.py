...
if __name__ == "__main__":
    ...
    # after a 1-second delay to ensure FastMCP completes its handshake first
    def pre_load_retriever():
        print("[server] Background pre-load thread: sleeping 1s to let FastMCP finish handshake...", file=sys.stderr, flush=True)
        time.sleep(1)
        ...
        try:
            ...
            set_load_status("Preparing environment and starting library preloading...")
            get_retriever(prevent_block=False)
            set_load_status("Finalizing RAG pipeline initialization...")
